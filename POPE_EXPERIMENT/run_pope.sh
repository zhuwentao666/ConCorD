...
for i in "${!MODEL_PATHS[@]}"; do
    ...
    if [ "$USE_ConCorD" = true ] && [ "$USE_DOLA" = true ]; then
        echo "错误：USE_ConCorD 和 USE_DOLA 不能同时为 true。"
        exit 1
    fi
    ...
    if [ "$USE_ConCorD" = true ]; then
        ...
        ANSWERS_FILE="./${SAVE_DIR}/${MODEL_NAME}_POPE_coco_results_${METHOD_STR}.jsonl"
        ...
        ANSWERS_FILE="./${SAVE_DIR}/${MODEL_NAME}_t${TEMPERATURE}_beam${NUM_BEAMS}POPE_coco_results_baseline.jsonl"
    fi
    ...
    # 打印执行信息
    echo "运行Qwen-VL POPE评测 - 模型: ${MODEL_NAME}"
    echo "结果文件: ${ANSWERS_FILE}"
    if [ "$USE_ConCorD" = true ]; then
        echo "方法: ConCorD (ALPHA=${ALPHA})"
    elif [ "$USE_DOLA" = true ]; then
        echo "方法: DOLA"
    else
        echo "方法: Baseline"
    ...
    # 运行评测
    OUTPUT_DIR='POPE_score'
    if [ -z "$ANSWERS_FILE" ] || [ -z "$OUTPUT_DIR" ]; then
        echo "错误：ANSWERS_FILE 或 OUTPUT_DIR 未设置"
    else
        echo "开始POPE评测..."
        python3 evaluate_POPE.py "$ANSWERS_FILE" "$OUTPUT_DIR"
        echo "POPE评测完成"
    fi
