...
from transformers import Qwen2_5_VLForConditionalGeneration, AutoProcessor, GenerationConfig, LlavaForConditionalGeneration, AutoModelForCausalLM, AutoTokenizer,AutoModelForImageTextToText
...
def eval_model(args):
    ...
    if args.use_flash_attn:
        if 'Qwen2.5-VL' in args.model_path:
            ...
            processor = AutoProcessor.from_pretrained(args.model_path, trust_remote_code=True, max_pixels=1003520)
        elif 'llava' in args.model_path:
    ...
    # 设置DECO相关参数
    gen_config.use_deco = args.use_deco
    gen_config.early_exit_layers = list(range(args.start_layer, args.end_layer))
    gen_config.alpha = args.alpha if args.use_deco else 0.0
    gen_config.threshold_top_p = args.threshold_top_p if args.use_deco else 0.0
    gen_config.threshold_top_k = args.threshold_top_k if args.use_deco else 0
    ...
    assert not args.use_dola or not args.use_deco
    if args.use_dola:
    ...
    with open(answers_file, "w", encoding="utf-8") as ans_file:
        for line in tqdm(data):
            ...
            # 生成响应
            output_text = generate_response(model, processor, filename, prompt, gen_config)
            ...
            # 保存结果
            res_dict = line
            res_dict['model_prediction'] = output_text
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_deco", action="store_true", default=False)
