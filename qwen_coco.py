...
from transformers import Qwen2_5_VLForConditionalGeneration, AutoProcessor, GenerationConfig, AutoModelForCausalLM, AutoTokenizer,AutoModelForImageTextToText,LlavaNextProcessor,LlavaNextForConditionalGeneration,LlavaForConditionalGeneration
from transformers import InstructBlipProcessor, InstructBlipForConditionalGeneration, AutoModel
...
def generate_response(model, processor, image_file, prompt, gen_config):
    ...
    with torch.no_grad():
        if 'Qwen-VL-Chat' in args.model_path:
            ...
            output_text = output_text[0]
        elif 'llama3-llava-next-8b-hf' in args.model_path:
            messages = [
                {
                    "role": "user",
                    "content": [
...
def eval_model(args):
    ...
    if args.use_flash_attn:
        if 'Qwen2.5-VL' in args.model_path:
            ...
            model = LlavaNextForConditionalGeneration.from_pretrained(args.model_path, torch_dtype=torch.float16, device_map="auto") 
            model.config.num_hidden_layers = 32
    ...
    # 读取COCO数据集,json格式
    with open(args.coco_data_file, "r", encoding="utf-8") as f:
        data = json.load(f)
    ...
    # 读取llava_results/greedy.jsonl
    # with open('llava_results/greedy.jsonl', 'r', encoding='utf-8') as f:
    #     lines = f.readlines()
    # # 解析JSONL文件，获取里面的id列表
    # ids = []
    # for line in lines:
    #     data_1 = json.loads(line)
    #     ids.append(data_1['id'])
    # # 过滤data，只保留id在ids中的数据
    # data = [item for item in data if item['image_id'] in ids]
    
    with open(answers_file, "w", encoding="utf-8") as ans_file:
        for line in tqdm(data):
            idx = line["image_id"]
            image_file = os.path.join(args.coco_image_path, f"COCO_val2014_{str(idx).zfill(12)}.jpg")
            
            # 准备提示词
            prompt = args.prompt if args.prompt else "Please describe this image in detail."
            ...
            # 保存结果
            res_dict = {"id": idx, "response": response}
